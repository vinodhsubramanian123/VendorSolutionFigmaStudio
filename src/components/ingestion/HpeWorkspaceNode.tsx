        ...
        <div className="flex justify-between items-start text-left">
          <div>
            <span className="text-[8px] bg-sky-500/10 text-sky-400 border border-sky-500/20 px-2 py-0.5 rounded font-black tracking-wide uppercase flex items-center gap-1 w-max inline-block">
              <span className={`w-1 h-1 rounded-full ${hpeSyncedConfigs === 4 ? "bg-status-success" : "bg-sky-400 animate-ping"} inline-block`} />
            ...
            <p className="text-[10px] text-content-secondary">HPE High-Core Blades</p>
          </div>
          <div className="text-right font-mono">
            <p className="text-xs font-bold text-content-primary">${(hpeSyncedConfigs * 105450).toLocaleString()}</p>
            <p className="text-[9px] text-content-primary0">tracked value</p>
          </div>
        </div>
        <div className="p-3 rounded bg-surface-canvas/10 border border-white/[0.03] space-y-2 text-left font-mono">
          <p className="text-[9px] text-content-secondary uppercase block">Sequential Execution Line ({ucidId})</p>
          <div className="space-y-1.5 pt-1 text-[10px]">
            {HPE_CONFIGS.map((cfg, idx) => (
              <div key={cfg.label} className="flex items-center justify-between p-1.5 rounded bg-surface-card border border-white/5">
                <span className="text-content-secondary">Config {idx + 1}: {cfg.label}</span>
      ...
      <div className="pt-4 border-t border-white/5 text-left">
        ...
        <div className="flex items-center gap-2 mt-2 font-mono">
          <div className="flex-1 h-1.5 bg-surface-canvas/30 rounded-full overflow-hidden">
            <div className="h-full bg-brand-indigo transition-all duration-300" style={{ width: `${(hpeSyncedConfigs / 4) * 100}%` }} />
          </div>
          <span className="text-[10px] text-content-primary font-bold leading-none">{hpeSyncedConfigs}/4 Synced</span>
        </div>
      </div>
    </div>
  );
