...
export function BreadcrumbNav({ view, activeMissionId, ucids, onNavigate }: BreadcrumbNavProps) {
  ...
  const activeMission = (view === 'live-mission' || view === 'taxonomy') && activeMissionId
    ? ucids.find(u => u.id === activeMissionId)
    : null;

  return (
    <div className="flex items-center text-[10px] font-mono tracking-wider mb-4 px-1 rounded bg-[#0b1220]/50 py-2 border border-white/5 shrink-0">
      <button 
      ...
      {activeMission && (
        <>
          <ChevronRight className="w-3 h-3 mx-2 text-gray-600" />
          <span className="uppercase text-[#ff9b36] font-bold">
            [{activeMission.displayId}] {activeMission.name.split('—')[0]?.trim() || activeMission.name}
          </span>
        </>
      )}
    </div>
