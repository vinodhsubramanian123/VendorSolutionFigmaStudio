...
import { Activity, Database, ChevronDown, ChevronUp, RefreshCw, Layers } from 'lucide-react';
...
export function StateConsistencyMonitor({ ucids, vendors, catalogSkus }: StateConsistencyMonitorProps) {
  ...
  useEffect(() => {
    ...
    console.log(`[STATE_MONITOR] | ${timestamp} | State Updated. Integrity Check:`, {
      ucids: { current: ucids?.length, previous: previousState.current.ucidsLength },
      vendors: { current: vendors?.length, previous: previousState.current.vendorsLength },
      catalogSkus: { current: catalogSkus?.length, previous: previousState.current.catalogSkusLength }
    });

    if (ucids?.length !== previousState.current.ucidsLength) {
      addLog('info', `UCID count changed: ${previousState.current.ucidsLength} -> ${ucids?.length || 0}`);
    }
    ...
    <div className="fixed bottom-4 right-4 w-80 bg-[#0b1220] border border-indigo-500/20 rounded-xl shadow-2xl shadow-indigo-900/20 z-50 flex flex-col overflow-hidden animate-fadeIn">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-white font-mono font-bold uppercase tracking-widest">State Cache Monitor</span>
        </div>
      ...
      <div className="p-4 flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-black/30 border border-white/5 rounded-lg p-3">
            <span className="text-[9px] text-gray-500 uppercase tracking-widest font-bold">Active UCIDs</span>
            <div className="flex items-end justify-between mt-1">
              <span className="text-xl font-mono text-white font-bold">{ucids?.length || 0}</span>
              <Layers className="w-4 h-4 text-indigo-400/50" />
            </div>
          </div>
          <div className="bg-black/30 border border-white/5 rounded-lg p-3">
            <span className="text-[9px] text-gray-500 uppercase tracking-widest font-bold">Local SKUs</span>
            <div className="flex items-end justify-between mt-1">
              <span className="text-xl font-mono text-white font-bold">{catalogSkus?.length || 0}</span>
              <Database className="w-4 h-4 text-[#00d4a0]/50" />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between">
            <span className="text-[9px] text-gray-400 uppercase tracking-widest font-bold">Event Log (Memory Tail)</span>
            <RefreshCw className="w-3 h-3 text-gray-500" />
          </div>
          <div className="h-32 bg-black/50 border border-white/5 rounded-lg p-2 overflow-y-auto space-y-2">
            {logs.length === 0 ? (
              <p className="text-[10px] text-gray-600 font-mono italic text-center mt-10">Monitoring state changes...</p>
            ) : (
              logs.map((log, i) => (
                <div key={i} className="flex gap-2 text-[9px] font-mono leading-relaxed">
                  <span className="text-gray-500 shrink-0">[{log.ts}]</span>
                  <span className={
                    log.level === 'error' ? 'text-[#ff3d5a]' :
                    log.level === 'warn' ? 'text-[#ff9b36]' :
                    'text-indigo-400'
                  }>{log.msg}</span>
                </div>
              ))
