      ...
      {isOpen && (
        ...
        <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between bg-surface/50">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-white">Add Part to Configuration</h2>
          </div>
        ...
        <div className="flex border-b border-white/5 px-6">
          <button
            ...
            className={`py-3 px-4 font-medium text-sm border-b-2 transition-colors ${
              activeTab === "catalog" ? "border-indigo-400 text-indigo-300" : "border-transparent text-gray-500 hover:text-gray-300"
            }`}
          >
            Search Catalog
          </button>
          <button
            ...
            className={`py-3 px-4 font-medium text-sm border-b-2 transition-colors ${
              activeTab === "custom" ? "border-indigo-400 text-indigo-300" : "border-transparent text-gray-500 hover:text-gray-300"
            }`}
          >
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {activeTab === "catalog" ? (
            <div className="space-y-4">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-black/40 border border-white/10 rounded-lg pl-10 pr-4 py-2.5 text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500/50"
                />
              </div>
              ...
              <div className="border border-white/5 rounded-lg overflow-hidden bg-black/20">
                <div className="max-h-64 overflow-y-auto">
                  {filteredCatalog.length === 0 ? (
                    <div className="p-8 text-center text-gray-500 text-sm">No parts found matching "{searchTerm}"</div>
                  ) : (
                    filteredCatalog.map(sku => (
                      <div 
                        ...
                        className={`p-3 border-b border-white/5 flex flex-col cursor-pointer transition-colors ${
                          selectedSkuId === sku.id ? "bg-indigo-500/10 border-indigo-500/30" : "hover:bg-white/5"
                        }`}
                      >
                        <div className="flex justify-between items-start">
                          <span className="font-mono text-sm text-indigo-300">{sku.partNumber}</span>
                          <span className="text-xs text-gray-500">{sku.type}</span>
                        </div>
                        <span className="text-sm text-gray-300 mt-1">{sku.name}</span>
                      </div>
                    ))
                  )}
                </div>
                ...
                <div className="flex items-center gap-4 pt-4 border-t border-white/5">
                  <div className="flex-1">
                    ...
                    <input
                      ...
                      className="w-32 bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-indigo-500/50"
                    />
                  </div>
                  <button 
                    onClick={handleCatalogAdd}
                    className="mt-5 px-6 py-2 bg-indigo-500 hover:bg-indigo-600 text-white font-medium rounded-lg transition-colors flex items-center gap-2"
                  >
            ...
            <div className="space-y-4">
              <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-lg text-sm text-amber-200/80 mb-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-1">
                  <label className="block text-xs text-gray-500 mb-1">Part Number (SKU)</label>
                  <input
                    type="text"
                    value={customPart}
                    onChange={(e) => setCustomPart(e.target.value)}
                    className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-indigo-500/50 font-mono"
                ...
                <div className="col-span-1">
                  ...
                  <input
                    ...
                    className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-indigo-500/50"
