...
import { FileSpreadsheet, Check, RefreshCw, ArrowRight, AlertTriangle } from 'lucide-react';

interface BoqIngestWorkbookProps {
  selectedPreset: 'hpe-legacy' | 'dell-overcharge' | 'cisco-asymmetry';
  setSelectedPreset: (val: 'hpe-legacy' | 'dell-overcharge' | 'cisco-asymmetry') => void;
  boqFile: string;
  isBOQIngesting: boolean;
  boqProgress: number;
  boqResponse: any;
  boqError: string;
  onTriggerBOQParse: (fileName: string, preset: 'hpe-legacy' | 'dell-overcharge' | 'cisco-asymmetry') => void;
        ...
        <div className="bg-[#0b1220] border border-white/5 rounded-xl p-6 space-y-6">
          <div className="flex justify-between items-start">
            <div>
              ...
              <h3 className="text-sm font-semibold text-white mt-1.5 animate-fadeIn">File Intake & Structural Splitting</h3>
            ...
            <div className="flex flex-col items-end gap-1 shrink-0">
              <span className="text-[10px] text-gray-400 font-mono">Simulate Document Profile:</span>
              <select
          ...
          >
            ...
            {isBOQIngesting ? (
              ...
              <div className="space-y-3 flex flex-col items-center">
                ...
                <div>
                  <p className="text-xs font-black text-emerald-300">{boqFile || 'Workbook-spec.xlsx'}</p>
              ...
              <div className="space-y-2 flex flex-col items-center text-gray-400 hover:text-white">
                <FileSpreadsheet className="w-9 h-9 text-gray-500 hover:text-sky-400 transition-colors" />
                <div>
            ...
            <div className="flex justify-center">
              <button
                ...
                onClick={() => {
                  ...
                  onTriggerBOQParse(matchedPreset?.file || 'Workbook.xlsx', selectedPreset);
                }}
                    ...
                    <div key={idx} className="flex justify-between items-center bg-[#070a13]/60 p-3 rounded border border-white/5 hover:border-white/10">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-gray-500 font-mono mt-0.5">{sol.items?.length || 0} hardware items mapped natively</p>
                        </div>
                      </div>
        ...
        <div className="bg-[#0b1220] border border-white/5 rounded-xl p-5 space-y-4">
          ...
          <div className="space-y-3 pt-2">
            <div className="flex items-start gap-2.5">
              ...
              <div>
                <span className="font-semibold text-white text-xs">Strict Vendor Decoupling</span>
                <p className="text-gray-500 text-[10px] mt-0.5">Configs are programmatically split by supplier types so they never taint or interfere with each other during procurement comparisons.</p>
              </div>
            </div>
            <div className="flex items-start gap-2.5">
              <div className="w-5 h-5 rounded-full bg-emerald-500/10 text-emerald-400 flex items-center justify-center shrink-0">
              ...
              <div>
                <span className="font-semibold text-white text-xs">Live Sourced Verification</span>
                <p className="text-gray-500 text-[10px] mt-0.5">Immediately registers active sessions via real REST APIs, saving developers from manually injecting complex data frames.</p>
              </div>
            </div>
          </div>
        </div>
