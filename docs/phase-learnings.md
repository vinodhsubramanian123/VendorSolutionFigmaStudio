...
## 29. Phase 3b/6 Backend Route Harmonization Learnings (July 2026)

### 29.1 The Permissive MSW Mock Trap
*   **Issue**: UI components frequently relied on permissive MSW mock handlers that either completely ignored request bodies or returned convenient, flattened responses instead of the strict nested structures mandated by the real `server.ts`. This allowed bugs like incorrect payload schemas or missing fields to pass local development and test assertions silently.
*   **Solution**: Never trust that a passing MSW test means the network boundary is safe. Always validate API caller payloads against their exact Zod schemas (e.g. `validateBody(ConstraintCheckRequestSchema)`) before crossing the boundary, and strictly assert the response shape against what the real backend returns, not the convenience objects generated by `msw`.

### 29.2 Validating Before the Boundary
*   **Issue**: Loose types like `string` for `JobContext.solution_id` were passed directly into API payloads where an enum was expected. MSW ignored the violation, but the real backend rejected it with 400s.
*   **Solution**: Always execute schema validation at the UI boundary before making an API call:
    ```typescript
    // BAD
    apiClient.ingest({ type: context.solution_id })
    // GOOD
    const safeType = IngestRequestSchema.shape.presetType.safeParse(context.solution_id)
    apiClient.ingest({ type: safeType.success ? safeType.data : 'default' })
    ```

### 29.3 Revert-and-Confirm-Fail Test Discipline
*   **Issue**: Tests were written to cover new backend logic, but occasionally used loose regexes that still matched the faulty code's output. If a test passes on the first try, it might be passing for the wrong reasons.
*   **Solution**: Always employ a **revert-and-confirm-fail** methodology. Before declaring a regression test valid, revert the fix, assert that the test actually fails against the bug, and then re-apply the fix to verify it passes. This prevents false confidence from inadequate assertions.

### 29.4 Playwright UX Overlap Enforcement (No "force: true")
*   **Issue**: End-to-End tests often used `.click({ force: true })` to bypass Playwright's strict actionability checks. This masked actual UI bugs where critical buttons (like "Split Configs") were obscured by overlapping z-index elements, modals, or toasts in specific views (e.g., the Cleansing View).
*   **Solution**: **NEVER** use `{ force: true }` in `.click()` calls unless interacting with an intentionally visually hidden DOM element. Playwright's default `.click()` automatically ensures the element is visible, stable, and not obscured by other elements. Removing `force: true` guarantees that the test accurately validates the true UX and catches layout overlap bugs.
