...
export function useDiffConfigs(
  ...
  const diffConfigs = useMemo(() => {
    ...
    const { snapA, snapB } = resolveSnapshots(selectedForCompare, compareAgainstCurrent, snapshotsList, activeUCID);

    if (!snapA || !snapB) return { snapA: null, snapB: null, sheets: [] };

    const comparisonList = computeDiffSheets(snapA, snapB);
...
function resolveSnapshots(
  ...
  snapshotsList: Snapshot[],
  activeUCID: UCID
): { snapA: Snapshot | null; snapB: Snapshot | null } {
