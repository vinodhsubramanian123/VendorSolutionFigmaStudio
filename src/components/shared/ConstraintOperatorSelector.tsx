...
export function ConstraintOperatorSelector({
  ...
  return (
    <div className="flex flex-col gap-2 p-3 bg-surface-elevated/50 border border-white/5 rounded-lg">
      ...
      <div className="flex bg-surface-canvas/40 p-1 rounded-lg border border-white/5 mt-1">
        <button type="button"
          onClick={() => onChange('AND')}
          className={`flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-md text-xs font-bold transition-all ${
            operator === 'AND'
              ? 'bg-brand-indigo text-content-primary shadow-md shadow-indigo-500/20'
      ...
      <div className="bg-brand-indigo/10 border border-brand-indigo/20 rounded-md p-2 mt-1">
