    ...
    <div className="flex flex-col gap-6 w-full text-left">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-[#0b1220] border border-white/5 rounded-xl p-5 space-y-4 lg:col-span-1">
          ...
          {targetUcid && (
            <div className="p-3.5 rounded-lg bg-[#070a13] border border-white/5 space-y-2 font-sans text-left">
              ...
              <div className="flex justify-between items-center pt-1 text-[10px]">
                <span className="text-gray-400 font-mono">Original solution:</span>
                <span className="font-bold text-white font-mono">{targetUcid.solutions[0]?.vendor}</span>
              </div>
        ...
        <div className="lg:col-span-3 space-y-4 text-left">
          <div className="bg-[#0b1220] border border-white/5 rounded-xl p-6 space-y-6">
            <div>
              <span className="text-[9px] bg-sky-500/10 text-sky-400 border border-sky-500/20 px-2 py-0.5 rounded uppercase font-extrabold tracking-wider">
              ...
              {isBOMIngesting ? (
                <div className="space-y-3 flex flex-col items-center">
                  ...
                  <div className="w-48 h-1 bg-white/5 rounded-full overflow-hidden text-left">
                    <div className="h-full bg-sky-500 transition-all duration-200" style={{ width: `${bomProgress}%`, transition: 'width 0.2s' }} />
                  </div>
                </div>
              ) : bomReconResult ? (
                <div className="space-y-2 flex flex-col items-center">
                  ...
                  <div>
                    <p className="text-xs font-black text-emerald-300">{activeBOMFile || 'supplier-bom.xlsx'}</p>
                    <p className="text-[9px] text-gray-500 font-mono mt-0.5 uppercase">Reconciliation audit successfully synthesized</p>
                  </div>
                </div>
              ) : (
            ...
            {bomReconResult && (
              <div className="space-y-6 pt-4 border-t border-white/5 animate-fadeIn text-left">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 font-mono text-[10px] text-left">
                      ...
                      <div className="p-3 bg-black/30 rounded border border-white/5 space-y-2 text-left">
                        ...
                        <div className="text-[10px] space-y-1">
                          <p className="text-white font-bold">{bomVerifyResult.socketMatch?.chassisSocket} Sockets matched.</p>
                          <p className="text-gray-500 leading-normal">{bomVerifyResult.socketMatch?.description}</p>
                        </div>
                      </div>
                      ...
                      <div className="p-3 bg-black/30 rounded border border-white/5 space-y-2 text-left">
                        ...
                        <div className="text-[10px] space-y-1">
                          <p className="text-white font-bold">Estimated: {bomVerifyResult.powerLimitTest?.estimatedTdpWatts} Watts</p>
                          <p className="text-gray-500">Max limit: {bomVerifyResult.powerLimitTest?.maxSupportedWatts}W</p>
                          <p className="text-emerald-400">Safety margin: {bomVerifyResult.powerLimitTest?.marginWatts}W</p>
                        </div>
                      </div>
                      ...
                      <div className="p-3 bg-black/30 rounded border border-white/5 space-y-2 text-left">
                        ...
                        <div className="text-[10px] space-y-1">
                          <p className="text-white font-bold">DDR5 Lanes Count: {bomVerifyResult.memoryBalanceCheck?.quantity}</p>
                          <p className="text-gray-500">Optimal Layout: {bomVerifyResult.memoryBalanceCheck?.optimalLayoutSymmetry}</p>
                          <p className="text-amber-400 text-[9.5px] leading-tight">{bomVerifyResult.memoryBalanceCheck?.message}</p>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-[#070a13] rounded-lg border border-white/5 p-4 space-y-4 text-left">
                    <span className="text-[10px] text-gray-400 font-mono block uppercase tracking-wider font-extrabold flex items-center gap-1.5 text-left">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      {/* Metric Box 1 */}
                      <div className="p-4 rounded-lg bg-[#070a13] border border-white/5 flex gap-3 items-center text-left">
